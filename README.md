...
- **Logo** is still a placeholder graphic. **Product photography** for the Book Now Card, Connect Card and Menu Stand comes from supplied design references (`src/data/products.js` → `images`); the homepage's animated 3D card keeps the real card's proportions (86.4mm × 54mm) and recreates the Book Now design in CSS rather than using a static photo, since it spins.
